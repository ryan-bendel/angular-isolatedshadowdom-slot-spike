...
function createRootTView(rootSelectorOrNode, componentDef, componentBindings, directives) {
  const tAttributes = rootSelectorOrNode ? ['ng-version', '21.1.0-next.0+sha-60acf6d-with-local-changes'] : extractAttrsAndClassesFromSelector(componentDef.selectors[0]);
...
function ɵɵprojectionDef(projectionSlots) {
  ...
  const componentNode = declarationComponentView[T_HOST];
  ...
  if (componentDef?.encapsulation === ViewEncapsulation.ExperimentalIsolatedShadowDom) {
    if (typeof ngDevMode !== 'undefined' && ngDevMode) {
      throw new Error(`ng-content projection is not supported with ViewEncapsulation.IsolatedShadowDom. ` + `Use native <slot> elements instead. Content will remain in the light DOM and be projected via slots.`);
    }
    return;
  }
